...
import { ChevronLeft, Heart, Sparkles, PlayCircle, ChevronRight, Eye, ChevronDown, AlertTriangle, Landmark, Box } from 'lucide-react';
...
const ProductDetailPage: React.FC = () => {
  ...
  const allMedia = useMemo(() => {
    if (!product) return [];
    const media: { type: 'image' | 'video' | '3d'; url: string }[] = product.imageUrls.map(url => ({ type: 'image', url }));
    if (product.videoUrl) {
      media.push({ type: 'video', url: product.videoUrl });
    }
    if (product.modelUrl) {
        media.push({ type: '3d', url: product.modelUrl });
    }
    return media;
  }, [product]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
        ...
        <div className="md:sticky md:top-28">
             <div 
              ...
              className={`relative group ${material === 'gold' ? 'material-gold' : ''}`}
                          ...
                          <div key={index} className="w-full h-full flex-shrink-0">
                              {media.type === 'video' ? (
                                  ...
                                  <ModelViewer
                                      ...
                                      className="w-full h-full bg-stone-100 dark:bg-stone-800"
                                  ></ModelViewer>
                              ) : (
                                  <ImageZoom
                                      key={media.url}
            ...
            <div className="flex overflow-x-auto space-x-3 mt-4 pb-2">
              {allMedia.map((media, index) => (
                <button
                  key={index}
                  onClick={() => setActiveMediaIndex(index)}
                  className={`w-20 h-20 rounded-md overflow-hidden border-2 transition-all duration-300 ease-in-out flex-shrink-0 focus-ring relative bg-stone-100 dark:bg-stone-800 ${activeMediaIndex === index ? 'border-[--color-gold-dark]' : 'border-transparent hover:border-stone-400 dark:hover:border-stone-500 scale-100 hover:scale-105'}`}
                  aria-label={`View media ${index + 1} of ${product.name}`}
                >
                  {media.type === 'video' ? (
                      <>
                        <img src={product.imageUrls[0]} alt="Video thumbnail" className="w-full h-full object-cover opacity-50" />
                        <div className="absolute inset-0 flex items-center justify-center">
                            <PlayCircle size={28} className="text-stone-800 dark:text-white"/>
                        </div>
                      </>
                  ) : media.type === '3d' ? (
                      <div className="w-full h-full flex items-center justify-center">
                          <Box size={28} className="text-stone-500 dark:text-stone-300" />
                      </div>
                  ) : (
                      <img src={media.url} alt={`${product.name} thumbnail ${index + 1}`} className="w-full h-full object-cover" loading="lazy"/>
                  )}
                </button>
              ))}
            </div>
        ...
        <div>
          ...
          <p className="text-3xl text-[--color-gold-dark] mt-4 font-bold">{formatPrice(product.price)}</p>
           ...
           <div className="my-6">
            <span className="text-sm font-semibold text-stone-600 dark:text-stone-400 me-3">Material:</span>
            <div className="inline-flex items-center bg-stone-100 dark:bg-stone-800 rounded-full p-1">
                <button 
                    onClick={() => setMaterial('silver')}
                    className={`px-4 py-1.5 text-sm font-semibold rounded-full transition-all duration-300 focus-ring ${material === 'silver' ? 'bg-white dark:bg-stone-700 shadow' : 'text-stone-500 hover:text-stone-800 dark:text-stone-400 dark:hover:text-white'}`}
                >
                    Silver
                </button>
                <button 
                    onClick={() => setMaterial('gold')}
                    className={`px-4 py-1.5 text-sm font-semibold rounded-full transition-all duration-300 focus-ring ${material === 'gold' ? 'bg-white dark:bg-stone-700 shadow' : 'text-stone-500 hover:text-stone-800 dark:text-stone-400 dark:hover:text-white'}`}
                >
                    Gold
                </button>
            </div>
