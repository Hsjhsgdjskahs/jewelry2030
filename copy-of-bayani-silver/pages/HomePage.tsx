...
const VipHeroSection: React.FC = () => {
    const context = useContext(AppContext);
    const { t } = useI18n();

    const titleMap = {
        'Bronze': t('themeMorph.vipBronzeTitle'),
        'Silver': t('themeMorph.vipSilverTitle'),
        'Gold': t('themeMorph.vipGoldTitle'),
    };

    const title = context?.vipLevel ? titleMap[context.vipLevel] || t('themeMorph.vipTitle') : t('themeMorph.vipTitle');

    return (
        <section className="relative h-[75vh] bg-gradient-to-br from-stone-800 to-stone-900 flex items-center justify-center text-center text-white overflow-hidden">
            <div className="absolute inset-0 bg-black/50 z-10"></div>
            <div className="absolute inset-0 z-0 opacity-20" style={{ backgroundImage: 'radial-gradient(var(--color-primary) 1px, transparent 1px)', backgroundSize: '20px 20px' }}></div>

            <div className="relative z-20 p-8">
                <div className="inline-block p-4 bg-white/10 rounded-full mb-4 border border-white/20 animate-fade-in">
                    <Gem size={40} className="text-[--color-primary]" />
                </div>
                <AnimatedHeadline text={title} as="h1" className="text-5xl md:text-6xl font-serif font-bold tracking-tight golden-shimmer-text" />
                <p className="mt-4 text-xl max-w-2xl mx-auto text-stone-200 animate-fade-in" style={{ animationDelay: '600ms'}}>{context?.vipMessage}</p>
                <div className="animate-fade-in" style={{ animationDelay: '800ms'}}>
                    <Link
                        to="/products"
                        className="mt-8 inline-block btn-primary-gradient text-white font-bold py-3 px-8 rounded-md text-lg transform active:translate-y-0 shadow-lg focus-ring breathing-effect"
                    >
...
const HomePage: React.FC = () => {
  ...
  return (
    <div className="space-y-24">
      {/* Hero Section */}
      {context?.vipLevel && context.vipLevel !== 'None' ? (
        <VipHeroSection />
      ) : prediction ? (
        ...
        <section className="relative h-[75vh] bg-white dark:bg-stone-900 flex items-center justify-center text-center text-white overflow-hidden">
            ...
            <div className="absolute inset-0 bg-black/50 z-10"></div>
            <div className="relative z-20 p-8">
            <AnimatedHeadline text={welcomeMessage?.title || ''} as="h1" className="text-5xl md:text-7xl font-serif font-bold tracking-tight golden-shimmer-text" />
